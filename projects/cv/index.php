  ...
  foreach ($positions as $position) {
      ...
      if (!in_array($enterprise, $displayedEnterprises)) {
          ...
          foreach ($groupedPositions[$enterprise] as $enterprisePosition) {
              ...
              echo '<div class="title" style="display:inline-block">' . $enterprisePosition['title'] . '</div><div class="duration" style="display:inline-block">' . $startDate->format('m-Y') . ' - ' . $endDate->format('m-Y') . ' (' . $this->calculateDuration($enterprisePosition['start'], $enterprisePosition['end']) . ')</div>';
              echo '';
